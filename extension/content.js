...
function groupRepos(selector, title, spawnPoint, options) {
	...
	if (map.size) {
		...
		map.forEach((users, repoUrl) => {
			...
			listEl.appendChild(fromHTML(`
				<li class="public source ghgn-stargazers-${options.stargazers}">
					...
					<div class="ghgn-stargazers">${options.stargazers === 'none' ? '' : usersHTML(users)}</div>
				</div>
			</li>`));
		});
